...
START_TEST(s21_round_2) {
  ...
  s21_round(test_1_dec, &result_dec);
  s21_from_decimal_to_int(result_dec, &result);
...
Suite *suite_round(void) {
  Suite *a = suite_create("Testing S21_ROUND [Other]");
  TCase *test_1, *test_2, *test_3, *test_4;

  test_1 = tcase_create("TEST1");
  tcase_add_test(test_1, s21_round_1);
  suite_add_tcase(a, test_1);

  test_2 = tcase_create("TEST2");
  tcase_add_test(test_2, s21_round_2);
  suite_add_tcase(a, test_2);

  test_3 = tcase_create("TEST3");
  tcase_add_test(test_3, s21_round_3);
  suite_add_tcase(a, test_3);

  test_4 = tcase_create("TEST3");
  tcase_add_test(test_4, s21_round_4);
